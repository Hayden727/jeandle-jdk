...
| ```JeandleDumpRuntimeStubs``` | bool | Dumps the compilation artifacts for JIT-internal **runtime stubs** (e.g., safepoint polls). When enabled, it dumps the pre-optimization IR, post-optimization IR, and the final object file for these stubs. This is distinct from ```JeandleDumpIR```/```Objects``` which target Java methods. | ```false``` |